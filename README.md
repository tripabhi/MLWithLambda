...
```bash
lambda-ml-a$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-ml-a$ sam local start-api
lambda-ml-a$ curl http://localhost:3000/classify_digit
```
...
```bash
lambda-ml-a$ sam logs -n InferenceFunction --stack-name lambda-ml-a --tail
```
...
```bash
aws cloudformation delete-stack --stack-name lambda-ml-a
```
